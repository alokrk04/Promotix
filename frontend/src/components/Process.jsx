...
export default function Process({ content }) {
  ...
  return (
    <section id="process" className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-transparent via-violet/5 to-transparent">
      <div className="section-shell max-w-6xl">
        ...
        <div className="relative max-w-[820px] mx-auto">
          ...
          {items.map((step, i) => (
            <div key={i} className="flex flex-col sm:flex-row gap-4 sm:gap-8 items-start pb-8 sm:pb-11 last:pb-0">
              <div className={`w-16 h-16 sm:w-20 sm:h-20 min-w-[64px] sm:min-w-[80px] rounded-full flex items-center justify-center text-base sm:text-lg font-extrabold border-2 relative z-10 bg-gradient-to-br ${stepColors[i % stepColors.length]}`}>
                {step.num || String(i + 1).padStart(2, '0')}
              </div>
              <div className="pt-0 sm:pt-3">
                <div className="text-lg sm:text-xl font-bold mb-2">{step.name}</div>
